...
findPhotoAll conn = do 
  s <- prepareStmt conn "SELECT p.id, d.id, d.name, d.bread, p.title, p.url, u.id, u.name from photos p inner join users u on p.user_id = u.id inner join dogs d on p.dog_id = d.id"
